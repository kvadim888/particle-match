...
4. Паралельна оцінка всіх частинок через адаптер `ParallelFor` (бекенд: TBB або `std::execution`)
...
```
Оцінка конфігурацій: для кожної афінної матриці обчислює відстань до шаблону. Фотометрично інваріантний режим нормалізує по середньому та стандартному відхиленню. Паралелізм керується через `ParallelFor` з вибраним бекендом.
...
- ParallelFor (бекенд: TBB або `std::execution`) -- паралельне обчислення
