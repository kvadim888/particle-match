...
У поточній реалізації основний алгоритм (`ParticleFastMatch`) використовує адаптер `ParallelFor` (бекенд TBB або `std::execution`) для паралельної оцінки частинок, а `FastMatcherThread` залишається як альтернативний механізм паралельності.
