...
int main(int argc, const char * argv[])
{
    ...
    fast_match.init( 0.1f, 0.85f, false, 0.5f, 2.0f );

    double distance;
    high_resolution_clock::time_point t1 = high_resolution_clock::now();
    fast_match.apply(image, templ, distance, -M_PI);
    fast_match.calculate();
    vector<Point2i> corners = fast_match.getBestCorners();
    ...
    imwrite("match-output.png", appended);
