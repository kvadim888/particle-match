...
namespace fast_match {
    ...
    vector<Point2f>
    FAsTMatch::apply(Mat &original_image, Mat &original_template, double &best_distance, float min_rotation,
                     float max_rotation) {
        ...
        while (true) {
            ...
            if (level > 3) {
                float mean_value =
                        std::accumulate(best_distances.begin() + level - 3, best_distances.begin() + level - 1, 0) *
                        1.0 / distances.size();
            ...
            vector<MatchConfig> good_configs = getGoodConfigsByDistance(configs, best_distance, new_delta, distances,
